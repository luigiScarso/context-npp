...
\starthead {\ConTeXt: Process TeX File} 
...
\starthead {\ConTeXt: Process TeX Project} 
...
\starthead {\ConTeXt: Process TeX File (fast)} 
...
\starthead {\ConTeXt: Process TeX Demo} 
...
\starthead {\ConTeXt: Process TeX File (module)} 
...
\starthead {\ConTeXt: Generate Format} 
...
\starthead {TEX: Scratch TeX File} 
...
\starthead {PDF: View PDF File (project)} 
...
\starthead {VIEW: Generate Listing} 

\starttyping
mtxrun --autogenerate --script context --extra=listing --pretty --result="$(NAME_PART)" "$(NAME_PART).tex" 
sumatra.bat "$(CURRENT_DIRECTORY)\$(NAME_PART).pdf"
\stoptyping
\stophead 

\starthead {Fonts: Purge Font Cache} 
...
\starthead {Fonts: OpenType Tables -- Lua} 
...
\starthead {Fonts: Generate font.tma} 
...
% \stopsubsection
\startsubsection[title={\NPPEXEC{} and the Macro Submenu},reference={}]
From the \NPPEXEC{} Advanced Options dialog, any console script can be made to appear at the bottom of the \type{Macro} submenu.We have configured 14 \NPPEXEC{} scripts appear there; see \in{Figure}[nppexec3]. Note the submenu name need not be the same as the source console script name!
